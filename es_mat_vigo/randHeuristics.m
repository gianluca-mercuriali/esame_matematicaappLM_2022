...
function [scarto_t_rand, scarto_m_rand, Profit_scarto_rand, matrix_t_rand, matrix_m_rand, Profit_rand] = randHeuristics(time,memory,Np,T,M,n_scaffali )

%% EURISTICA RANDOMIZZATA

matrix_t_rand = zeros (n_scaffali,Np);
matrix_m_rand = zeros (n_scaffali,Np);
...
for j = 1:n_scaffali %ciclo che scorre i n_scaffali scaffali (ho diviso area totale in n_scaffali scaffali)
    ...
    while  1 %ciclo che mi riempie lo scaffale corrente fino a saturare i tempi
        ...
        if((tx + temp_t_rand(i) > T) || (mx + temp_m_rand(i) > M/n_scaffali)) %condizione che controlla che non vengano oltrepassati i limiti di memoria e tempo del cassetto corrente
            break
        else
